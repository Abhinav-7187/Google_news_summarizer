...
# Get Groq API key, Serper API key, number of results, and search query
with st.sidebar:
    ...
    st.caption("*Search & Summarize: Uses Serper & Groq APIs, summarizes each search result.*")
...
if col1.button("Search"):
    # Validate inputs
    if not groq_api_key.strip() or not serper_api_key.strip() or not search_query.strip():
           ...
           with st.spinner("Please wait..."):
                ...
                if not result_dict['news']:
                    ...
                    for i, item in zip(range(num_results), result_dict['news']):
                        ...
                        # Initialize the ChatGroq module, load and run the summarize chain
